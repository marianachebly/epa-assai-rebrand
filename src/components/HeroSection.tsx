import bannerAniversario from "@/assets/banner-aniversario66.png";
import FloatingBalloons from "@/components/FloatingBalloons";

const HeroSection = () => {
  return <section className="relative min-h-[480px] overflow-hidden" style={{
    backgroundColor: '#f9e103'
  }}>
      <FloatingBalloons />
      <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,')] opacity-20"></div>
      
      <div className="container mx-auto px-4 relative z-10 flex items-center justify-center min-h-[480px]" style={{ paddingTop: '25px', paddingBottom: '-23px' }}>
        <img src={bannerAniversario} alt="EPA 66 Anos - Meio Milhão em vales-compra de R$ 1.000" className="w-full max-w-5xl max-h-[480px] object-cover object-top" />
      </div>
    </section>;
};
export default HeroSection;